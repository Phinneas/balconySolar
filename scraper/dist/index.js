...
// src/teable-client.js
var TeableClient = class {
  ...
  async getRecords(tableId, filter4 = null) {
    let endpoint = `/table/${tableId}/record`;
    if (filter4) {
      endpoint += `?filter=${encodeURIComponent(filter4)}`;
    }
...
__name(TeableClient, "TeableClient");
var teable_client_default = TeableClient;
...
var RegulationScraper = class {
  ...
  async scrapeState(stateCode) {
    ...
    try {
      const html3 = await this.fetchHTML(config.url);
      const regulations = await config.parser(html3);
      return {
  ...
  async parseNewYorkRegulations(html3) {
    const $2 = load(html3);
    const content = $2.text();
    return {
      isLegal: content.includes("solar") && !content.includes("prohibited"),
      maxWattage: 1200,
      // New York allows up to 1200W
  ...
  async parseTexasRegulations(html3) {
    const $2 = load(html3);
    const content = $2.text();
    return {
      isLegal: content.includes("solar") && !content.includes("prohibited"),
      maxWattage: 1e3,
      // Texas allows up to 1000W
      keyLaw: "PURA \xA7 49.452",
      details: {
        interconnection: {
          required: false,
...
__name(RegulationScraper, "RegulationScraper");
var scraper_default = RegulationScraper;
...
__name(invalidateAPICache, "invalidateAPICache");
async function updateStateInTeable(client, stateData, oldStateData = null) {
  try {
    ...
    const hasChanges = stateRecord.fields.isLegal !== stateData.isLegal || stateRecord.fields.maxWattage !== stateData.maxWattage || stateRecord.fields.keyLaw !== stateData.keyLaw;
    if (hasChanges) {
...
__name(updateStateInTeable, "updateStateInTeable");
...
__name(updateStateDetails, "updateStateDetails");
...
__name(updateStateResources, "updateStateResources");
...
__name(sendErrorNotification, "sendErrorNotification");
async function handleScheduled(event, env) {
  const startTime = Date.now();
  const jobId = `scraper-${(/* @__PURE__ */ new Date()).toISOString()}`;
  console.log(`[${jobId}] Starting scheduled scraper job`);
  const teableClient = new teable_client_default(
    env.TEABLE_API_URL,
    env.TEABLE_BASE_ID,
    env.TEABLE_API_TOKEN
  );
  const scraper = new scraper_default();
  try {
